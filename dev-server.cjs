...
const { findAvailablePort } = require('./scripts/find-available-port.cjs');
let PORT = process.env.PORT || process.env.FUNCTIONS_PORT || 8888;
...
async function startServer() {
  try {
    // If PORT is not a number or not available, find an available port
    if (isNaN(PORT)) {
      PORT = await findAvailablePort(8888);
    } else {
      // Check if the specified port is available
      const { isPortAvailable } = require('./scripts/find-available-port.cjs');
      if (!(await isPortAvailable(PORT))) {
        console.log(`⚠️  Port ${PORT} is busy, finding alternative...`);
